...
def update_latest_market(db_number, data_sourced_from,main_loop, archive_data):
    ...
    try:
        ...
        with open(os.path.join(r'C:\Users\PC\PycharmProjects\Simcompanies\Files\MarketResults',
                               'Market for ' + str(db_number)) + '.txt', "r") as file1:
            old_data = eval(file1.read())
            file1.close()
    except:
        old_data = []
        print("ERROR File not found, continuing:" + str(str(db_number)) + '.txt')
    ...
    # print('Final output' + str(formatted_sell_list))
...
loops = 1000 # This is, roughly, how many mins to run for. In reality it will be about 6% more than this.
...
if data_source == 'live':
    for i in range(loops):
        ...
        print('End of loop ' + str(i) + ', total time taken was: ' + str(total_time) + ' for ' + str(hits_this_loop) + ' products')
        times_report = []
